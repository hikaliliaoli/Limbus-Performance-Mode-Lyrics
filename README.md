...
- 标准模式会随大字号自动扩展窗口到屏幕允许的范围；超过屏幕物理尺寸的长句会等比缩小以避免裁切。
- `锁定歌词位置`：标准模式下取消锁定后，每一条可见歌词都能单独拖动；拖动空白区域仍可移动整个悬浮窗。重新锁定后所有歌词恢复鼠标穿透。
...
默认使用标准模式：歌词按独立行排列并逐字显示；取消锁定后可分别拖动。`Top` 设置为 `-1` 时使用自动窗口位置。
