...
一个轻量的 Windows 桌面歌词工具。它从网易云本地播放日志或 Windows 媒体会话读取当前歌曲，优先按网易云歌曲 ID 查询官方逐行歌词，并用透明、无边框、始终置顶且鼠标穿透的“演出字幕”显示。
...
- 歌词查询需要联网。读取到网易云本地日志中的歌曲 ID 时，程序会直接按该 ID 获取网易云原文和翻译；没有歌曲 ID 时会先按歌名、歌手搜索网易云。只有网易云没有可用歌词或请求失败时，才使用 LRCLIB 的公开同步歌词作为备用来源。
